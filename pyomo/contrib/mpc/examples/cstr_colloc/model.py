...
def initialize_model(m, dynamic=True, ntfe=None, ncp=None, clean_model='delete'):
    if ntfe is not None and not dynamic:
        ...
        ntfe = 10
    if ncp is not None and not dynamic:
        raise RuntimeError("Cannot provide ncp to initialize steady model")
    elif dynamic and ncp is None:
        ncp = 3
    if dynamic:
        disc = pyo.TransformationFactory("dae.collocation")
        disc_kwargs = dict(wrt=m.time, nfe=ntfe, ncp=ncp, scheme="LAGRANGE-LEGENDRE")
        if clean_model is not None:
            disc_kwargs["clean_model"] = clean_model
        disc.apply_to(m, **disc_kwargs)
...
def create_instance(dynamic=True, horizon=None, ntfe=None, ncp=None, clean_model='delete'):
    if horizon is None and dynamic:
        horizon = 10.0
    if ntfe is None and dynamic:
        ntfe = 10
    if ncp is None and dynamic:
        ncp = 3
    m = make_model(horizon=horizon, dynamic=dynamic)
    initialize_model(m, ntfe=ntfe, ncp=ncp, dynamic=dynamic, clean_model=clean_model)
    return m
